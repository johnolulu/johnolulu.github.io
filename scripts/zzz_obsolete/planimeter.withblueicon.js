...
Planimeter.prototype.mapMouseMove = function ( cursorLatLng ) {
  ...
  if ( this.points.length != 0 ) {
    ...
    // Remove any previously displayed active line:
    ...
    if ( this.points.length >= 3 ) {
      ...
      // Set start point to location of first marker:

      var end = this.points[ 0 ];

      // Define second polyline:

      this.activeLine2 = new GPolyline( [ cursorLatLng, end ], this.closerColor, this.lineWidth, this.lineOpacity, { geodesic: true } );
...
Planimeter.prototype.display =  function () {
  ...
  // Clear all previously displayed active (cursor-following) lines:
...
Planimeter.prototype.markerClickAction = function ( object, pointIndex ) {
  ...
  if ( object.markerDoubleClicked ) {
    ...
    if ( object.mapClickListener ) {
      ...
      if ( object.mapMouseMoveListener ) {
        GEvent.removeListener( object.mapMouseMoveListener );
        object.mapMouseMoveListener = null;
      }

      if ( object.activeLine1 ) {
        object.map.removeOverlay( object.activeLine1 );
      }

      if ( object.activeLine2 ) {
        object.map.removeOverlay( object.activeLine2 );
      }
      ...
      if ( object.htmlElement ) {
        object.htmlElement.style.color     = 'GOLD';
        object.htmlElement.style.fontStyle = 'normal';
        object.htmlElement.title           = 'Measure area between three or more points; click on map to set points...';
      }
