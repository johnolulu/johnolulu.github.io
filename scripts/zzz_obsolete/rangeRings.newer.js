...
rangeRings.prototype.safeRedraw = function() {
  ...
  // Define click event for marker:

  var markerAction = GEvent.callback( this, this.markerAction );

  this.listenClick = GEvent.addListener( marker, 'click', 
    function () {
      markerAction(); 
    }
  );  
...
This function displays an info pop-up window on a marker with its lat/lon.
*/

rangeRings.prototype.markerAction = function () { 

  // Get location of marker:

  var latLon = this.centre.toString();
  ...
  markerHtml  = '<p class="googleMapAddressText">lat, lon:</p>'
              + '<p><font color="#000000">'
              + lat + ', ' + lon
              + '</font></p>';

  this.map.openInfoWindowHtml( this.centre, markerHtml );
}
