...
function rangeRings ( point, interval, maxRings, labels ) {
...
rangeRings.prototype.copy = function () {
  return new rangeRings( this.point, this.interval, this.maxRings, this.labels );
}
...
rangeRings.prototype.safeRedraw = function() {
  ...
  if ( d.constructor.toString().indexOf( 'Array' ) == -1 ) {
    ...
    if ( this.maxRings == null ) {
      ...
      for ( var r = d; ( r < ( diagKm * 2 ) ); r += d ) {
        this.drawCircle( r, r.toFixed( p ) + 'km' );
      }
    }
    else {
      ...
      for ( var i = 0; i < this.maxRings; i++ ) {
        if ( this.labels == null ) {
          ...
          this.drawCircle( r, this.labels[ i ] );
        }
        r += d;
      }
    }
  ...
  // Interval array used:
    ...
    for ( var i = 0; i < d.length; i++ ) {
      ...
      if ( this.labels == null ) {
        this.drawCircle( d[ i ] / 1000.0, d[ i ].toFixed( p ) + 'km' ); 
      }
      else {
        this.drawCircle( d[ i ] / 1000.0, this.labels[ i ] );
      }
...
rangeRings.prototype.drawCircle = function ( rKm, label ) {
  ...
  var c = new GPolyline( pts, colour, weight, opacity );
