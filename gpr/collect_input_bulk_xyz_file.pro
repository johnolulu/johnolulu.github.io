...
;;  XYZ file (latitude, longitude, depth) of a layer identified within one or more RAMAC
;;  ground-penetrating radar (GPR) data files. An "XYZ file" is a text file with three space-
...
;;      called within "create_xyz_file.pro". When selecting multiple data files (e.g.
...
;;      acceptable. The GroundVision Standard format (*.cor) is a comma-delimited text file with
...
;;      file. When subsetting, therefore, it is necessary to record the xstart of every
;;      subset/transect so that these files record their location within the original
;;      *.rd3 RAMAC data file and can find their associated GPS coordinates in the *.cor file.
;;
...
PRO collect_input_bulk_xyz_file, event
    ...
    gpr_files = ENVI_PICKFILE( title = "Select the GPR file(s) to generate an XYZ file for:", /MULTIPLE_FILE, $
        filter = "*.rd3" )
    ...
    ; Let the user select the RAMAC GPS file (*.cor) associated with the selected GPR file(s):

    gps_file = ENVI_PICKFILE( title = "Select the RAMAC GPS file (*.cor) associated with the selected GPR file(s):", $
